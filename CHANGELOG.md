...
- 修复：插件类型轨道的属性卡里，数字 / 文本字段在点击卡片外时未提交，导致更改不记录、不保存（现在关闭卡片前会先提交聚焦的输入）
- 修复：复制踩点未复制插件属性值（`attrs`），粘贴后变为默认值
- 内部：`updateMarkerAttrs` 改为 Pinia action，撤销 / 操作记录与其它编辑保持一致
- 安装包：`Beat-Data-Generator-0.2.15-setup.exe`
